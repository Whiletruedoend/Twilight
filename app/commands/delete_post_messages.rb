class DeletePostMessages
  ...
  def call
    if channel_id.nil?
      ...
      telegram_posts = post.platform_posts.joins(:platform).where(platforms: { title: "telegram"}, channel_id: channel_id)
      matrix_posts = post.platform_posts.joins(:platform).where(platforms: { title: "matrix"}, channel_id: channel_id)
      title = Channel.find_by_id(channel_id).platform.title
      case title
        when "telegram"
          delete_telegram_posts(telegram_posts) if telegram_posts.any?
        when "matrix"
      ...
      platform = Platform.find_by_title(title)
    ...
    telegram_posts.each do |platform_post|
      begin
        ...
        if platform_post.content.has_attachments?
          platform_post.identifier.each { |att| bot.delete_message({ chat_id: att["chat_id"], message_id: att["message_id"] }) }
        else
          bot.delete_message({ chat_id: platform_post[:identifier]["chat_id"], message_id: platform_post[:identifier]["message_id"] })
        end
